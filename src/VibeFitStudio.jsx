...
function getNextPaymentLabel(m, payments) {
  ...
  if(m.planType==="monthly"){
    ...
    for(let i=0;i<WEEKLY_DATES.length;i++){
      ...
      if(wEnd<td) continue;
      const paid=mp.some(p=>
        (p.planType==="weekly"||p.planType==="individual"||p.planType==="separo")
        &&p.date>=WEEKLY_DATES[i]&&p.date<=wEnd
      );
...
function isPaid(m,payments,d){
  ...
  // Semanal O clase individual en la misma semana = semana cubierta
  const idx=getWeekIdxForDate(d);
  if(idx<0)return false;
  const wStart=WEEKLY_DATES[idx],wEnd=getWeekEnd(idx);
  return mp.some(p=>
    (p.planType==="weekly"||p.planType==="separo"||p.planType==="individual")
...
.hdr h1{font-family:'Fredoka',sans-serif;font-size:22px;font-weight:700;color:#fff;letter-spacing:.5px}
.hdr-sub{font-size:12px;color:rgba(255,255,255,.8);margin-top:1px;font-weight:500}
.hdr-row{display:flex;justify-content:space-between;align-items:center}
.sync{display:inline-flex;align-items:center;gap:4px;padding:3px 8px;border-radius:12px;font-size:10px;font-weight:700;color:#fff;background:rgba(255,255,255,.2)}
...
.nbdg{position:absolute;top:1px;right:calc(50% - 18px);min-width:16px;height:16px;border-radius:8px;background:var(--rs);color:#fff;font-size:9px;font-weight:800;display:flex;align-items:center;justify-content:center;padding:0 3px}
...
.sv{font-family:'Fredoka';font-size:24px;font-weight:700}
.sl{font-size:10px;color:var(--tx2);margin-top:1px;text-transform:uppercase;letter-spacing:.8px;font-weight:700}
.si{position:absolute;top:10px;right:10px;font-size:20px}
.bdg{display:inline-flex;align-items:center;gap:3px;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700}
.bg{background:var(--mtb);color:var(--mt)}.br{background:var(--rdb);color:var(--rd)}.bo{background:var(--gdb);color:var(--gd)}.bb{background:var(--blb);color:var(--bl)}.bpk{background:var(--pks);color:var(--pk)}
.sb{display:inline-flex;align-items:center;gap:2px;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:800;font-family:'Fredoka'}
.s7{background:#FFE0E8;color:#E85D75}.s8{background:#E8F2FF;color:#4A90D9}.s9{background:#F3E8FF;color:#9B6BC5}
.btn{display:inline-flex;align-items:center;gap:5px;padding:11px 16px;border-radius:var(--rs2);border:none;font-family:'Nunito';font-size:14px;font-weight:700;cursor:pointer;transition:all .12s}
...
.bsm{padding:7px 12px;font-size:12px}.bfull{width:100%;justify-content:center}
.fab{position:fixed;bottom:96px;right:max(16px,calc(50% - 224px));width:54px;height:54px;border-radius:50%;background:linear-gradient(135deg,var(--pk),var(--rs));color:#fff;border:none;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 20px rgba(255,107,138,.4);cursor:pointer;z-index:90}
.fab:active{transform:scale(.88) rotate(-90deg)}
.fg{margin-bottom:13px}
.fl{display:block;font-size:11px;color:var(--tx2);margin-bottom:4px;text-transform:uppercase;letter-spacing:.5px;font-weight:700}
.fi{width:100%;padding:11px 13px;background:var(--sf2);border:1.5px solid var(--bd);border-radius:var(--rs2);color:var(--tx);font-size:15px;font-family:'Nunito';outline:none;transition:border .2s;-webkit-appearance:none}
.fi:focus{border-color:var(--pk);box-shadow:0 0 0 3px rgba(255,107,138,.12)}
select.fi{cursor:pointer}
.mo{position:fixed;inset:0;background:rgba(61,43,53,.45);backdrop-filter:blur(4px);z-index:200;display:flex;align-items:flex-end;justify-content:center;animation:fadein .2s}
.mob{background:var(--sf);border-radius:22px 22px 0 0;width:100%;max-width:480px;max-height:88vh;overflow-y:auto;padding:20px;animation:slideup .28s cubic-bezier(.34,1.4,.64,1)}
.moh{display:flex;align-items:center;justify-content:space-between;margin-bottom:18px}
.mot{font-family:'Fredoka';font-size:18px;font-weight:700;color:var(--pk)}
.mox{width:32px;height:32px;border-radius:50%;background:var(--sf2);border:1px solid var(--bd);color:var(--tx2);display:flex;align-items:center;justify-content:center;cursor:pointer}
...
.srch input{width:100%;padding:11px 13px 11px 38px;background:var(--sf);border:1.5px solid var(--bd);border-radius:var(--r);color:var(--tx);font-size:14px;font-family:'Nunito';font-weight:600;outline:none;box-shadow:var(--sh)}
.srch input:focus{border-color:var(--pk)}
.li{display:flex;align-items:center;gap:11px;padding:13px 16px;border-bottom:1px solid var(--sf3);cursor:pointer;transition:background .1s}
.li:active{background:var(--sf2)}
.la{width:42px;height:42px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:800;font-size:16px;flex-shrink:0;font-family:'Fredoka'}
.lc{flex:1;min-width:0}.ln{font-size:15px;font-weight:700}.ls{font-size:12px;color:var(--tx2);margin-top:2px;font-weight:500}
.lr{text-align:right;flex-shrink:0}.lam{font-family:'Fredoka';font-size:15px;font-weight:700}
...
.dr{display:flex;justify-content:space-between;align-items:center;padding:11px 0;border-bottom:1px solid var(--sf3)}
.dr:last-child{border-bottom:none}
.dl{font-size:13px;color:var(--tx2);font-weight:600}.dv{font-size:14px;font-weight:700}
.tabs{display:flex;gap:3px;padding:4px;margin:10px 16px 6px;background:var(--sf2);border-radius:var(--r);border:1px solid var(--bd)}
...
.empty{text-align:center;padding:36px 20px;color:var(--tx3)}.empty p{margin-top:6px;font-size:14px;font-weight:600}
.acts{display:flex;gap:7px;margin-top:12px}.acts .btn{flex:1}
.sg{display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px}
.so{padding:12px 6px;border:2px solid var(--bd);border-radius:var(--rs2);text-align:center;cursor:pointer;background:var(--sf)}
.so.on{border-color:var(--pk);background:var(--pks)}
.sot{font-family:'Fredoka';font-size:16px;font-weight:700}
.pg{display:grid;grid-template-columns:1fr 1fr;gap:7px}
.po{padding:11px 8px;border:2px solid var(--bd);border-radius:var(--rs2);text-align:center;cursor:pointer;background:var(--sf)}
.po.on{border-color:var(--pk);background:var(--pks)}
.pon{font-size:12px;font-weight:700}.pop{font-family:'Fredoka';font-size:16px;font-weight:700;color:var(--pk);margin-top:1px}.poi{font-size:18px}
.br2{display:flex;align-items:center;gap:6px;margin-bottom:5px}
.bl2{font-size:10px;font-weight:800;color:var(--tx3);width:28px;text-align:right;font-family:'Fredoka'}
...
.cfmb{background:#fff;border-radius:var(--r);padding:22px;max-width:300px;width:88%;text-align:center}
.cfmb h3{font-family:'Fredoka';font-size:16px;margin-bottom:6px}.cfmb p{font-size:13px;color:var(--tx2);margin-bottom:14px;line-height:1.4}
.cfa{display:flex;gap:7px}.cfa .btn{flex:1}
.mid{font-family:'Fredoka';font-size:11px;font-weight:700;color:var(--tx3);letter-spacing:1px}
...
.wa-tpl{padding:9px 11px;border:1.5px solid var(--bd);border-radius:var(--rs2);background:var(--sf);cursor:pointer;margin-bottom:7px;transition:border .15s}
...
.alta-toggle{display:flex;margin-bottom:14px;border:1.5px solid var(--bd);border-radius:var(--rs2);overflow:hidden}
...
/* Opciones de pago próximo */
.pay-opt{padding:11px 12px;border:2px solid var(--bd);border-radius:var(--rs2);cursor:pointer;background:var(--sf);margin-bottom:7px;display:flex;align-items:center;justify-content:space-between;transition:border .15s}
.pay-opt.on{border-color:var(--pk);background:var(--pks)}
.pay-opt.monthly{border-left:4px solid #4A90D9}
.pay-opt.weekly{border-left:4px solid #E85D75}
.pay-opt.on.monthly{border-color:#4A90D9;background:#E8F2FF}
.pay-opt.on.weekly{border-color:#E85D75;background:var(--pks)}
.pay-opt-lbl{font-size:13px;font-weight:700;color:var(--tx)}
.pay-opt-price{font-family:'Fredoka';font-size:15px;font-weight:700;color:var(--pk)}
.pay-opt-sub{font-size:10px;color:var(--tx2);margin-top:2px}
/* Próximo pago badge en lista */
.next-pay{font-size:10px;font-weight:700;padding:2px 7px;border-radius:8px;display:inline-block;margin-top:3px}
.next-pay.monthly{background:var(--blb);color:var(--bl)}
.next-pay.weekly{background:var(--pks);color:var(--rs)}
.next-pay.ok{background:var(--mtb);color:var(--mt)}
/* Calendario */
.cal-nav{display:flex;align-items:center;gap:8px;padding:12px 16px 6px}
.cal-nav-btn{width:36px;height:36px;border-radius:50%;border:1.5px solid var(--bd);background:var(--sf);display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0}
.cal-nav-lbl{flex:1;text-align:center;font-family:'Fredoka';font-size:17px;font-weight:700;color:var(--tx)}
.cal-wrap{padding:0 0 16px}
.cal-box{background:var(--sf);border-top:1px solid var(--bd);border-bottom:1px solid var(--bd);overflow:hidden}
...
.cal-hdr-title{font-family:'Fredoka';font-size:18px;font-weight:700;color:#fff;letter-spacing:1px}
.cal-pills{display:flex;gap:7px}
.cal-pill{display:flex;align-items:center;gap:4px;background:rgba(255,255,255,.22);color:#fff;font-size:10px;font-weight:700;padding:3px 9px;border-radius:10px;font-family:'Fredoka'}
.cal-pill-dot{width:8px;height:8px;border-radius:50%}
.cal-legend{display:flex;gap:14px;padding:8px 16px 6px;flex-wrap:wrap}
.cal-leg{display:flex;align-items:center;gap:5px;font-size:11px;font-weight:700;color:var(--tx2)}
.cal-leg-dot{width:12px;height:12px;border-radius:50%;flex-shrink:0}
.cal-grid{display:grid;grid-template-columns:repeat(7,1fr);padding:4px 8px 10px;gap:2px}
.cal-dow{font-size:10px;font-weight:700;color:var(--tx3);text-align:center;padding:3px 0 6px;font-family:'Fredoka';text-transform:uppercase}
.cal-dow.wkday{font-weight:800;color:var(--tx2)}
.cal-detail{padding:6px 16px 12px;border-top:1px solid var(--sf3)}
.cal-detail-row{font-size:11px;font-weight:700;margin-bottom:3px}
...
function Cobros({state,go}){
  ...
  // Semana actual via WEEKLY_DATES
  const wIdx=getWeekIdxForDate(td);
  const wCobStart=wIdx>=0?WEEKLY_DATES[wIdx]:mon;
  const wCobEnd=wIdx>=0?getWeekEnd(wIdx):sun;
  const pW=all.filter(m=>(m.status||"active")==="active"&&m.planType==="weekly"&&!pays.some(p=>
    p.memberId===m.id&&
    (p.planType==="weekly"||p.planType==="individual"||p.planType==="separo")&&
    p.date>=wCobStart&&p.date<=wCobEnd
  ));
